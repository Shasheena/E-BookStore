...
if($scopy["type"] == "application/pdf"){
    if(isset($image)){
        ...
        if(!in_array($image["type"],$img_type_array)){
            ...
            Database::iud("INSERT INTO `books`(`title`,`authors_id`,`sub_categories_id`,`image`,`usage_id`,`publisher_id`,`qty`,`discount`,`shipping`,`delivery_fee_colombo`,`delivery_fee_other`,`pdf`,`copy_copy_id`,`cover_id`,`price`) VALUES
            ('".$title."','".$author_id."','".$cg."','".$new_image_file."','".$usage."','".$publisher_id."','".$qty."','".$dsc."','".$shipping."','".$dfc."','".$dfo."','".$new_name."','".$copy."','".$ct."','".$price."')");
