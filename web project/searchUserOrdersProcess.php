...
if(empty($orderId) && empty($dFrom) && empty($dTo)){
    echo("Seach something!");
}else{
    if (!empty($orderId) && empty($dFrom) && empty($dTo)) {
        $query .= "`order_id`='" . $orderId . "'";
        // echo(1);
    } else if (empty($orderId) && !empty($dFrom) && empty($dTo)) {
        $query .= "`user_email`='" . $_SESSION["user"]["email"] . "' AND `date` >= '" . $dFrom . "'";
        // echo(2);
    } else if (empty($orderId) && empty($dFrom) && !empty($dTo)) {
        $query .= "`user_email`='" . $_SESSION["user"]["email"] . "' AND `date` <= '" . $dTo . "'";
        // echo(3);
    } else if (!empty($orderId) && !empty($dFrom) && empty($dTo)) {
        $query .= "`user_email`='" . $_SESSION["user"]["email"] . "' AND `order_id`='" . $orderId . "' AND `date` >= '" . $dFrom . "'";
        // echo(4);
    } else if (!empty($orderId) && empty($dFrom) && !empty($dTo)) {
        $query .= "`user_email`='" . $_SESSION["user"]["email"] . "' AND `order_id`='" . $orderId . "' AND `date` <= '" . $dTo . "'";
        // echo(5);
    } else if (empty($orderId) && !empty($dFrom) && !empty($dTo)) {
        $query .= "`user_email`='" . $_SESSION["user"]["email"] . "' AND `date` BETWEEN '" . $dFrom . "' AND '" . $dTo . "'";
        // echo(6);
    } else if (!empty($orderId) && !empty($dFrom) && !empty($dTo)) {
