...
$query = "SELECT * FROM `books` INNER JOIN `authors` ON books.authors_id = authors.id WHERE ";

if($c_id=="no" && !empty($au) && !empty($title)){
        $query .= "`fname` LIKE '%".$au."%' OR `lname` LIKE '%".$au."%' AND `title` LIKE '%".$title."%'";
    // echo(1);
    ...
    $query .= "`sub_categories_id` = '".$c_id."'";
    // echo(3);
}else if ($c_id!="no" && !empty($au) && empty($title)){
    
        $query .= "`fname` LIKE '%".$au."%' OR `lname` LIKE '%".$au."%' AND `sub_categories_id` = '".$c_id."'";
   
    // echo(4);
}else if($c_id!="no" && !empty($au) && !empty($title)){
   
        $query .= "`fname` LIKE '%".$au."%' OR `lname` LIKE '%".$au."%' AND `sub_categories_id` = '".$c_id."' AND 
        `title` LIKE '%".$title."%'";
   
}else if ($c_id=="no" && !empty($au) && empty($title)){
   
        $query .= "`fname` LIKE '%".$au."%' OR `lname` LIKE '%".$au."%'";
...
<div class="col-12">
                <table class="table table-hover">
                    <thead>
                        <tr>
                            <th scope="col" style="color: orangered;">Book</th>
                            <th scope="col" style="color: orangered;">Title</th>
                            <th scope="col" style="color: orangered;">Copy</th>
                            <th scope="col" style="color: orangered;">Author</th>
                            <th scope="col" style="color: orangered;">Quantity</th>
                            <th scope="col" style="color: orangered;">Price (Rs.)</th>
                            <th scope="col" style="color: orangered;">Seller</th>
                            <th scope="col" style="color: orangered;">Seller Email</th>
                            <th scope="col" style="color: orangered;">Discount</th>
                            <th scope="col"></th>
                        </tr>
                    </thead>
