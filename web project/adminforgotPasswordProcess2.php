...
$verify_rs = Database::search("SELECT * FROM `admin` WHERE `verification_code` = '".$code."'");
$verify_num = $verify_rs->num_rows;

if($verify_num==1){
    $pw_rs = Database::search("SELECT * FROM `admin` WHERE `password` = '".$password."'");
    ...
    if($pw_num>0){
        ...
        if(strlen($password)<6){
            ...
            Database::iud("UPDATE `admin` SET `password` = '".$password."' WHERE `verification_code` = '".$code."'");
